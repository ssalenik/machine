static int pt_main(struct pt *pt) {
	...
	// robot must be placed 30 mm forward from transition 0,
	// with the actuator pointing left
	nav_actu(3, 400); PT_WAIT_UNTIL(pt, pid_complete[MOTOR4]);
	nav_base(20, 180); PT_WAIT_UNTIL(pt, ref_complete[MOTOR4]);
	set_rel_pos(0, 30);
	pos_corr_on();
	nav_rel_pos(DRIVE_SPEED, 17,  70); // start moving under the bridge
	...
	// positioned in front of the barrier here
	nav_base(10, -5);
	nav_actu(2, 40);
	...
	// end of fast turn
	nav_rel_pos(DRIVE_SPEED, 11,  20); // drive to pick up the battery
	...
	nav_base(10, 0);
	nav_actu(2, 300); PT_WAIT_UNTIL(pt, ref_complete[MOTOR4]);
	...
	nav_base(10, -82); // aim for target
	PT_WAIT_UNTIL(pt, drive_complete && pid_complete[MOTOR3] && pid_complete[MOTOR4]);
	SLEEP(400);
	cannon_shoot(); SLEEP(800);
	cannon_reload(); SLEEP(SERVO_DELAY);
	cannon_shoot(); SLEEP(800); // 2nd attempt just in case
	cannon_reload();
	nav_forward( 40, 80); SLEEP(300); // smooth acceletation
	nav_forward( 80, 80); SLEEP(300);
	nav_forward(120, 80); SLEEP(300);
	nav_forward(160, 80); SLEEP(300);
	nav_rel_pos(200, 35,  0); SLEEP(300); // go short-circuit metal plates
	nav_base(10, 30); PT_WAIT_UNTIL(pt, ref_complete[MOTOR3]);
	nav_base(10, 75);
	nav_actu(3, 200);
	PT_WAIT_UNTIL(pt, drive_complete);
	pt_target = uptime(); // stable short-circuit begins here
	nav_actu(2, 100); PT_WAIT_UNTIL(pt, pid_complete[MOTOR3] && pid_complete[MOTOR4]);
	clr_bit(FET1); clr_bit(FET2); SLEEP(200); // drop the battery
	nav_actu(3, 250); PT_WAIT_UNTIL(pt, ref_complete[MOTOR4]);
	nav_actu(3, 400);
	PT_WAIT_UNTIL(pt, uptime() > pt_target + 1500); // make sure short-circuit is at least 1.5 s
