...
/* NOT FOR ARDUINO: bootloader switch */
#define BL_DDR  DDRB
...
/* hacked by nCipher: only start the real app if the bootloader switch is de-asserted */
void app_start(void) {
	asm volatile("jmp\t0x1e000\n\t");
}

void real_app_start(void) {
...
int main(void)
{
	...
	/* however, we don't wanna confuse people, to avoid making a mess, we will stick to RXD1, TXD1 */
	bootuart = 2;
#endif
	...
	/* mods by nCipher: check bootloader switch */
	if(bit_is_set(BL_PIN, BL)) {
		real_app_start();
	}
	...
	if(bootuart == 1) {
		UBRR0L = (uint8_t)((F_CPU/BAUD_RATE + 8)/16-1);
		UBRR0H = ((F_CPU/BAUD_RATE + 8)/16-1) >> 8;
		UCSR0A = 0x00;
		UCSR0C = 0x06;
		UCSR0B = _BV(TXEN0)|_BV(RXEN0);
	}
	if(bootuart == 2) {
		UBRR1L = (uint8_t)((F_CPU/BAUD_RATE + 8)/16-1);
		UBRR1H = ((F_CPU/BAUD_RATE + 8)/16-1) >> 8;
		UCSR1A = 0x00;
		UCSR1C = 0x06;
		UCSR1B = _BV(TXEN1)|_BV(RXEN1);
	}
#elif defined __AVR_ATmega163__
	UBRR = (uint8_t)((F_CPU/BAUD_RATE + 8)/16-1);
	UBRRHI = ((F_CPU/BAUD_RATE + 8)/16-1) >> 8;
	UCSRA = 0x00;
	...
	UBRR0L = (uint8_t)((F_CPU/BAUD_RATE + 4)/8-1);
	UBRR0H = ((F_CPU/BAUD_RATE + 4)/8-1) >> 8;
#else
	UBRR0L = (uint8_t)((F_CPU/BAUD_RATE + 8)/16-1);
	UBRR0H = ((F_CPU/BAUD_RATE + 8)/16-1) >> 8;
#endif
...
#elif defined __AVR_ATmega8__
	/* m8 */
	UBRRL = (uint8_t)((F_CPU/BAUD_RATE + 8)/16-1);
	UBRRH = ((F_CPU/BAUD_RATE + 8)/16-1) >> 8;
