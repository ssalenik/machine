...
static int pt_test(struct pt *pt) {
	...
	forward_dist(50, 20); PT_WAIT_UNTIL(pt, drive_complete);
	stop(); sleep_test(500);
...
static int pt_main_flow(struct pt *pt) {
	...
	// START MANUAL
	forward(25); PT_WAIT_UNTIL(pt, is_falling(60.0)); // advance
	EM_enable(); sleep(1000); // robot is falling
	stop(); sleep(4000); // robot is hanging and stabilizing
	EM_disable(); sleep(3000); // drop and land
	// STOP MANUAL
...
void disable_rotadj() {
	fprintf_P(&drive, PSTR("4300\r"));
}

void enable_rotadj() {
	fprintf_P(&drive, PSTR("4301\r"));
}
